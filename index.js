...
console.log(fifaData.filter(play => play.Year === 2014).filter(play => play.Stage === "Final")[0]["Away Team Name"]);
console.log(fifaData.filter(play => play.Year === 2014).filter(play => play.Stage === "Final")[0]["Home Team Name"]);
console.log(fifaData.filter(play => play.Year === 2014).filter(play => play.Stage === 'Final')[0]["Away Team Goals"]);
console.log(fifaData.filter(play => play.Year === 2014).filter(play => play.Stage === "Final")[0]["Win conditions"]);
console.log(fifaData.filter(play => play.Year === 2014).filter(play => play.Stage === "Final")[0]["Home Team Goals"]);
...
function getYears(callback) {
    let years = callback.map(yearsObj => yearsObj.Year)
    return years
}
...
function getWinners(arr) {
    const newArr = arr.map(parametar => {
        if (parametar['Home Team Goals'] > parametar["Away Team Goals"]) {
            return (parametar["Home Team Name"])
        } else {
            return (parametar["Away Team Name"])
        }
    });
    return newArr;
}
...
function getWinnersByYear(getWinners, getYears) {
    ...
    for (let i = 0; i < getWinners.length; i++) {
        theWinner.push(`In ${getYears[i]}, ${getWinners[i]} won the world cup!`)
    }
    return theWinner
};
...
function getAverageGoals(arr) {
    const homeAvg = arr.map((element) => element['Home Team Goals'])
    const awayAvg = arr.map((element) => element['Away Team Goals'])
